...
export default makeScene2D(function* (view) {
    // === VIDEO SECTION ===
    const videoContainerRef = createRef<Node>();
    const videoRef = createRef<Video>();

    // === DEMO SECTION ===
    const demoContainerRef = createRef<Node>();
    ...
    view.add(
        <>
            ...
            <Node ref={videoContainerRef}>
                <Video
                    ref={videoRef}
                    height={view.height() - 190}
                    src={animation}
            ...
            <Node ref={demoContainerRef} x={600} scale={0.5} opacity={0}>
                <Rect
    ...
    // 2. Transition: video shrinks and moves left, demo appears and grows
    yield* all(
        // Video: shrink and move left
        videoContainerRef().scale(0.5, 0.8, easeInExpo),
        videoContainerRef().x(-500, 0.8, easeInExpo),
        videoContainerRef().opacity(0.3, 0.8),
        
        // Demo: appear and move to center
        delay(0.3, demoContainerRef().opacity(1, 0.6, easeOutExpo)),
        delay(0.3, demoContainerRef().x(0, 0.8, easeOutExpo)),
        delay(0.3, demoContainerRef().scale(1, 0.8, easeOutBack)),
    );

    // Hide video completely
    videoContainerRef().opacity(0);
    videoRef().pause();
...
const KeyRow = ({ keys, pad = 0, noContainer = false }: any) => {
    const content = keys.split("").map((char: string, i: number) => (
        <Rect
            key={i.toString()}
            width={42}
            height={42}
